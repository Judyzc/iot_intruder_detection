...
// static int run_face_recognition(fb_data_t *fb, std::list<dl::detect::result_t> *results)
// {
//     // Fast guard: only attempt recognition when PIR window is active
//     if (!pir_active) return -1;
...
// #endif
//     int face_id = 0;
//     if ( !(detection_enabled || pir_active) || fb->width > 400 )
//     {
// #endif
// #if ARDUHAL_LOG_LEVEL >= ARDUHAL_LOG_LEVEL_INFO
//         size_t fb_len = 0;
...
// // PIR gating
// #if CONFIG_ESP_FACE_DETECT_ENABLED
//         // If the ISR fired, arm the active window (non-ISR code)
//         // handle_pir_trigger_if_needed();
 
//         // If PIR not active, skip heavy detection path (fast path only)
//         if (!pir_active) {
//             goto send_frame;
//         }

//         // Expire detection window when time is up (use esp_timer_get_time for portability)
//         {
//             int64_t now_ms = esp_timer_get_time() / 1000;
//             if (now_ms >= pir_active_until_ms) {
//                 pir_active = false;
//                 ESP_LOGI(TAG, "PIR detection window expired; disabling recognition");
//                 goto send_frame;
//             }
//         }
// #endif
...
// //         // 2) ensure we have JPEG bytes in _jpg_buf/_jpg_buf_len
// //         if (fb->format == PIXFORMAT_JPEG) {
// //             _jpg_buf = fb->buf;
// //             _jpg_buf_len = fb->len;
// //             // We will return fb after sending
// //         } else {
// //             // convert to JPEG
// //             bool jpeg_converted = frame2jpg(fb, 80, &_jpg_buf, &_jpg_buf_len);
// //             esp_camera_fb_return(fb);
// //             fb = NULL; // converted buffer is in _jpg_buf now (malloc'd)
...
// //         // 3) send multipart boundary + headers + jpeg payload
// //         res = httpd_resp_send_chunk(req, _STREAM_BOUNDARY, strlen(_STREAM_BOUNDARY));
// //         if (res != ESP_OK) break;

// //         int hlen = snprintf(part_buf, sizeof(part_buf), _STREAM_PART, _jpg_buf_len, (int)_timestamp.tv_sec, (int)_timestamp.tv_usec);
...
// //         // 4) free/return buffers correctly
// //         if (fb) {
// //             // fb->buf is owned by fb, return it
// //             esp_camera_fb_return(fb);
// //             fb = NULL;
// //         } else if (_jpg_buf) {
// //             // we allocated _jpg_buf via frame2jpg, free it
// //             free(_jpg_buf);
// //             _jpg_buf = NULL;
// //         }
// //         _jpg_buf_len = 0;

// //         // 5) small delay to be friendly to CPU and network (adjust as needed)
...
// //     // clean up leftover buffer if loop exits
// //     if (fb) {
// //         esp_camera_fb_return(fb);
// //     }
// //     if (_jpg_buf) {
